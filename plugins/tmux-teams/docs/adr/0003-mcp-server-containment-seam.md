...
that matters for ข้อ 13 is the dispatched leg, and for that the claim holds.

This was never decided in the open. It happened to be true because nothing
ever asked the companion to send anything else, and ข้อ 13's list of
prohibitions never named it — so the code enforced a rule this document did
not state, which is the same kind of disagreement ข้อ 15.3 exists to catch, just
in the direction of an undocumented protection instead of an undocumented
gap. ข้อ 15.2 exists precisely so an unenforced clause SAYS so instead of
...
are mediated, attributed to an actor, and reviewable through ข้อ 3–ข้อ 9 of the
contract. Before that conversation happens, "closed" needs to be a decision
on the record, not a default nobody chose.

## Decision

**The seam stays closed.** `loop-system-contract.md` ข้อ 13 gains a prohibition:
...
document already defines (ข้อ 3 declaration contract, ข้อ 4 ledger, ข้อ 6 occupancy).
...
through anything ข้อ 4–ข้อ 9 already do.
...
it requires amending ข้อ 13's prohibition directly, and that amendment is
...
- `loop-system-contract.md` ข้อ 13 carries the prohibition. A dispatch that ever
  sends a non-empty `mcpServers` is a contract violation by definition,
  whether or not a test catches it on the day it happens.
- ~~ข้อ 14.1 records the specific gap this ADR does not close: nothing asserts the
...
- The next request to widen a dispatched agent's tool access via MCP starts
  ...
  sign-off and a ข้อ 13 amendment, not a config flag, an env var, or a
  `graph.json` field.
