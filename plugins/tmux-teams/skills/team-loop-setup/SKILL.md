...
The Team delivery flow page (`.tmux-teams/team-flow.html`, published by
...
1. **Start from the template or from scratch.**
   ...
   ```bash
   node <plugin>/skills/tmux-teams/scripts/team-flow.mjs init <repo>
   ```
...
4. **Check before dispatching anything.**
   ...
   ```bash
   node <plugin>/skills/tmux-teams/scripts/team-flow.mjs check <repo>
   ```
...
5. **Publish and look at it.**
   ...
   # then open <repo>/.tmux-teams/team-flow.html
   ```
