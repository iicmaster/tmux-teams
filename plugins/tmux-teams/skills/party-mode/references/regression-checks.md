...
- `TC-PA-31`: each lane receives the same static packet in a fresh temporary workspace, starts in the ACP mode its profile declares (plan for every lane but `zai`, which declares `default`) with ACP permission denial, and explicitly acknowledges its routed model/config; runner-owned evidence labels Kimi as `kimi/opus` and Zai as `zai/glm-5.2`. A routed lane also proves its own family: its `ANTHROPIC_BASE_URL` must match the host pinned in its profile, because the panel counts `family` to prove three distinct families reviewed the work and an operator-owned settings file could otherwise point two lanes at one provider.
