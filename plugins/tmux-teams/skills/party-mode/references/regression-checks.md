...
- `TC-PA-09`: ALL Codex subagents — thinking-heavy and execution-heavy alike — route to the current Codex frontier model (`gpt-5.6-sol` at `max` reasoning effort; Frontier always, 2026-07-14) with no Spark or inherited-default downgrade; non-Codex runtimes report equivalent routing or model-selection limits.
...
- `TC-PA-26`: a primary `codex`/`gpt` run routes exactly to AGY, Qwen `qwen3.8-max-preview`, and Zai GLM-5.2; Codex is absent from the final panel.
...
- `TC-PA-28`: a failed Qwen `qwen3.8-max-preview` or Zai GLM-5.2 lane may use only a gate-approved eligible reserve, and the final panel still has exactly three valid distinct reviewers including AGY.
...
- `TC-PA-30`: Claude routes to AGY + Codex + Qwen with a valid Zai reserve. Kimi routes to AGY + Codex + Zai, Zai routes to AGY + Codex + Qwen, and Qwen routes to AGY + Codex + Zai. Direct Claude remains limited because `claude-zai` would duplicate/retry Zai or match the primary where applicable; Gemini, AGY, and unknown primaries fail closed rather than being inferred by the PM.
- `TC-PA-31`: each lane receives the same static packet in a fresh temporary workspace, starts in the ACP mode its profile declares (plan for every lane but `zai`, which declares `default`) with ACP permission denial, and explicitly acknowledges its routed model/config; runner-owned evidence labels Qwen as `qwen/qwen3.8-max-preview` and Zai as `zai/glm-5.2`. Both `claude-qwen` and `claude-zai` use `@agentclientprotocol/claude-agent-acp`, and each routed lane proves its own family because `ANTHROPIC_BASE_URL` must match the host pinned in its profile.
