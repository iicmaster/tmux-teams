...
- Run it only through `node <party-mode>/scripts/review-gate.mjs <static-packet.json> <absolute-target-repository> > <review-report.json>`. Capture the trusted target path before reading the packet; never derive it from packet data. The gate builds a deterministic route, sends the same redacted static packet from a fresh temporary workspace, starts each reviewer in the ACP mode its profile declares — `plan` for every lane except `zai`, a route decision rather than a limitation: glm-5.2 was measured on 2026-08-13 returning one strict JSON document in BOTH modes on the same packet, plan mode faster and cleaner (87s/4027 bytes against 204s/4170), and `plugins/tmux-teams/skills/party-mode/scripts/review-profiles.mjs` records that measurement beside the pin — denies ACP permission requests, verifies the pinned model/config acknowledgement, and enforces its timeout (900s per lane, raised with `REVIEW_GATE_LANE_TIMEOUT_SEC`). Do not substitute `oc`, AGY/Codex review plugins, MCP review tools, or a host-agent review command.
- **There is no OS sandbox, and no way to ask for one.** ADR 0006's 2026-08-24 amendment removed the bwrap code rather than merely undeclaring it, so the gate runs on macOS and Linux alike. `osSandbox` is read by nothing: a grep for it across this skill’s shipped scripts returns no hit, so a profile that declares it is declaring a field with no consumer. This bullet said the opposite — that declaring it "gets the full sandbox" and that "turning it back on is one word" — through the release that removed it, and then described at length a confinement that no longer existed: masked host roots, an ephemeral provider HOME, a new PID namespace, and a tolerance rule for AGY built-in reads confined to an isolated `builtin/` tree. `inspectAgySafeRead` and that tree are gone too, so an AGY read is not tolerated as safe — it blocks the lane like any other tool call. A gemini review lane caught the whole paragraph on the v0.35.0 release diff; the head of the bullet had been updated and its body had not, which is the shape to watch for whenever a capability is removed. **What a lane actually gets, all of it enforced by the runner and the gate rather than by any OS feature:** a temporary workspace, no built-in tools requested, no MCP servers, every permission denied, `toolCallsObserved: 0` required, the endpoint pinned and verified in the PARENT before the child starts, the packet redacted both ways (sensitive keys plus bounded assignment, header and query-string values), and — before launch — the profile-owned executable resolved, target- and PATH-shadowed binaries rejected, and a trusted runtime root required. What was given up is OS-level filesystem confinement, and only that. Network is still shared for provider APIs. And nothing is COPIED any more: `prepareProviderState` creates an empty state home and returns, so the child inherits the real host `HOME` and the whole provider home is readable, not a minimal staged copy of the auth it needs — which is what `hostProviderHomeVisible: true` and `hostDataRootsMasked: false` in the evidence object have been saying all along. This bullet said "the adapter can read the minimum copied auth in its own ephemeral HOME", which understated the exposure; a deepseek review lane caught it on the v0.35.0 release diff, one round after the same rewrite fixed a different overstatement in the same paragraph. Treat same-process auth, the readable host home, and local/LAN reachability as residual boundaries, never as proof of zero live-service access.
- **A packet must forbid tool calls in its own words, and must not tempt them.**
  A review lane runs with `toolCallsObserved: 0` as an acceptance condition, so a
  lane that opens one file is refused and — when it is AGY, which cannot be
  replaced — the whole panel is blocked. Measured 2026-08-14: a documentation
  packet quoting many file paths was blocked exactly this way, on the first try,
  while the same three lanes had just answered two other packets. Paths in a
  packet read as an invitation. Say in the objective that every usable byte is in
...
|---|---|---|
| OpenAI / Codex / `gpt-*` | AGY + Qwen `qwen/qwen3.8-max-preview` + Zai `zai/glm-5.2` | none while direct Claude is limited; `claude-zai` would duplicate/retry Zai, so block |
| Claude | AGY + Codex + Qwen `qwen/qwen3.8-max-preview` | Zai `zai/glm-5.2` |
| Kimi | AGY + Codex + Zai `zai/glm-5.2` | none while direct Claude is limited; `claude-zai` would duplicate/retry Zai, so block |
| Zai | AGY + Codex + Qwen `qwen/qwen3.8-max-preview` | none while direct Claude is limited; `claude-zai` matches the primary family, so block |
| Qwen | AGY + Codex + Zai `zai/glm-5.2` | none while direct Claude is limited; `claude-zai` would duplicate/retry Zai, so block |
...
- Direct Claude is currently provider-limited and is never launched by the gate. `claude-zai` and `claude-qwen` are operator profile aliases behind the same `@agentclientprotocol/claude-agent-acp` transport; `claude-zai` is the deterministic availability reserve and launches the pinned Zai GLM-5.2 profile only if the resulting final panel remains valid. The default reviewer replacement for Kimi is Qwen, not a direct Claude launch.
...
- The gate records transport, runner-owned profile/provider labels, configured-and-acknowledged ACP model, route, packet hash, enforced isolation controls, and report structure. It does not cryptographically attest the remote serving model. `family` in the routing table above is a declared label from `<party-mode>/scripts/review-profiles.mjs`, not something observed from the ACP wire. Qwen and Zai both use the identical `@agentclientprotocol/claude-agent-acp` package through `claude-qwen` and `claude-zai`; each has a separate machine-local settings profile and a parent-verified endpoint pin before spawn. The legacy Kimi profile is no longer selected by the default reviewer routes, but it IS endpoint-pinned (`api.kimi.com`) exactly like the routed lanes — this line said "unpinned" until 2026-08-05 and the code disagreed. The reason it is pinned while unused is in the profile table itself: an unpinned lane on the shared `claude-agent-acp` package is indistinguishable from bare `claude`, so any future route seating both would count two families as three and the gate would say PASS. The gate additionally refuses any panel whose accepted lanes resolve to the same unpinned identity. The PM still reads the findings, checks the cited evidence, and decides semantic correctness. A finding independently raised by at least two of the final three reviewers is must-fix; other findings require recorded PM judgment.
...
The report must validate to exactly three distinct reviewers, include AGY, and acknowledge the routed model/profile for every lane. For OpenAI/Codex/`gpt-*`, expect AGY, Qwen, and Zai GLM-5.2; Codex is not eligible. For Claude, Kimi, Zai, or Qwen primary, use the canonical matrix above. Gemini, AGY, or unknown primary identity is blocked.
...
three_model_plan_review:
  ...
  reviewers:
    ...
    - lane: <qwen|zai-glm-5.2|eligible reserve>
      model: <acknowledged routed model>
      profile: <acknowledged routed profile>
      verdict: <pass|objections>
    - lane: <zai-glm-5.2|qwen|eligible reserve>
      model: <acknowledged routed model>
      profile: <acknowledged routed profile>
      verdict: <pass|objections>
...
For Codex critique agents, use the current latest Codex frontier model only, currently `gpt-6-sol` at `ultra` reasoning effort.
...
For Codex coding/refactoring/file-edit/implementation subagents, use the current latest Codex frontier model — currently `gpt-6-sol` at `ultra` reasoning effort, the same as thinking work (Frontier always; see Subagent Model Routing). Do not route execution to Codex Spark.
...
- A blocked gate still writes its report. Once the lanes launch, every blocker exits nonzero **and** writes a `blocked` report to stdout, so the `> <review-report.json>` redirect is never empty: `attempts[]` carries each lane's `profile`, `status`, `stage`, `reason`, `action`, `failure`, `stderrDigest` and `stderrBytes`, and `substitution` carries `used`, `replaced`, `replacement`, `remaining` and `reason`. `stage` is the transport's own bounded code — `spawn`, `config` (auth), `protocol`, `timeout`, `closed`, `review`, `policy`, `input`, `transport` — plus the two gate-local stages `acknowledge` (isolation/acknowledgement evidence) and `schema` (review validator). `reason` is a code from the gate's own closed table (`LANE_FAILURES` in `review-gate.mjs`, every entry deep-frozen so no same-process code can rewrite an action after import) and `action` is that code's fixed next step, repeated on stderr for the lanes that have one — a lane that fails with `packet_hash` is discarded and never retried, while one that fails with `schema_invalid` is re-run once, and those are different instructions that used to arrive as the same sentence. A rejection at a deterministic transport stage — `config`, `policy`, or `input` — reports `lane_rejected` ("do not retry, fix the profile") rather than `lane_failed` ("re-run and compare stderrDigest"): that advice used to be the same for a misconfigured lane and a merely slow one, and only one of those repairs itself on a second attempt. Read `action` before retrying anything. Raw provider stderr never crosses that boundary; only its digest and byte count do — a provider controls that text, the report is written to a file people paste into issues and chats, and the redactor is a denylist that cannot be proven complete (it does not catch an OAuth `code=` in a device-flow URL, which is exactly what an unauthenticated lane prints). A custom `validateReview`'s own reason text rides into `failure` as a bounded subject, printable ASCII only — a control byte, newline, or ANSI escape drops the subject entirely rather than forwarding it, since that seam is the caller's own validator, not the closed default vocabulary. A throwing fallback planner or synthesizer still produces this same structured `blocked` report instead of an empty stdout. A blocker raised *before* any lane launches — unreadable packet, unsupported or conflicting primary, undiversified panel — has no lane to report and writes only its stderr line.
...
- Model-routing drift: any Codex subagent — thinking or execution — uses anything other than `gpt-6-sol` at `ultra` reasoning effort, any Claude subagent uses anything other than Opus at maximum reasoning effort, or non-Codex routing silently misreports unavailable model selection.
