...
For Codex critique agents, use the current latest Codex frontier model only, currently `gpt-6-sol` at `max` reasoning effort.
...
For Codex coding/refactoring/file-edit/implementation subagents, use the current latest Codex frontier model — currently `gpt-6-sol` at `max` reasoning effort, the same as thinking work (Frontier always; see Subagent Model Routing). Do not route execution to Codex Spark.
...
- Model-routing drift: any Codex subagent — thinking or execution — uses anything other than `gpt-6-sol` at `max` reasoning effort, any Claude subagent uses anything other than Opus at maximum reasoning effort, or non-Codex routing silently misreports unavailable model selection.
