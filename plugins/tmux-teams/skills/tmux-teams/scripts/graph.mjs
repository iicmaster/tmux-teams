...
<ul class="keys">
 ...
 <span><b>A crawling edge means the seat it points AT is running</b> — one crawl per running <i>team</i> seat that an edge points at, keyed by seat and not by token. The outer controller is a running seat with no <i>assign</i>, <i>owns</i> or <i>judge</i> edge aimed at it, so a working controller draws no crawl at all; its state is on its own node border, not on a wire. Two teams interleaving on one team therefore light two edges around a single node: the incoming <i>assign</i> because that worker is running, and the outgoing <i>artifact</i> because that evaluator is running on a different token. That is <b>two tokens in flight</b>, not one artifact mid-handover. The runs this page reads carry a task id, never a work item, so it cannot say which token is which without being told.</span>
