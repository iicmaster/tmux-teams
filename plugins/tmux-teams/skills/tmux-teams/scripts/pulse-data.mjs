...
// These sets are the READ vocabulary, mirroring the frozen `pulse-v*.schema.json`
// enums exactly. Nothing emits `DELIVERY_LOOP_*` or `DELIVERY_RUNTIME_*` any
// more — those projections and their producers are gone — but a pulse.json an
// installation already has on disk may carry one, and `clonePulseDiagnosticForCompat`
// REJECTS the whole snapshot on an unknown code. Dropping them here would make
// `compat-v1` refuse a file it can read today, which is a regression, not a
// cleanup. They stay until the schemas themselves are unfrozen.
...
export function projectPulseV3(view, meta) {
  const diagnostics = [...(view.diagnostics || [])]
  diagnostics.sort((left, right) => {
    const priority = (code) => code === 'PHASE_BINDING_CONFLICT' ? 0
      : code === 'PHASE_BINDING_INVALID' ? 1
        : code === 'SCHEMA_UPGRADED' ? 2 : 3
...
export function projectPulseV4(
  ...
  diagnostics.sort((left, right) => {
    const priority = (code) => code?.startsWith('TEAM_RUNTIME_')
      ? 0
      : code?.startsWith('TEAM_GRAPH_') ? 1
        : code?.startsWith('LIVENESS_EVIDENCE_') ? 2
          : code === 'PHASE_BINDING_CONFLICT' ? 3
            : code === 'PHASE_BINDING_INVALID' ? 4
              : code === 'SCHEMA_UPGRADED' ? 5 : 6
    return priority(left?.code) - priority(right?.code)
  })
...
// There is no `downProjectPulseV2`. The v2 schema REQUIRES a `delivery_loop`
// projection, and nothing in this repo can produce one any more — the only
// input it could ever accept would be a stub hand-built by its own test. A
// down-projector to a shape no producer can reach is not compatibility, it is
// a shadow. Reading a v2 file already on disk is a different question and is
// still answered: `downProjectPulseV1` tolerates the key without validating it,
// and `priorStream` upgrades such a file to v4 by version alone.
