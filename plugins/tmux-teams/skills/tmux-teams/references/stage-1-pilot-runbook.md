...
operator projection แบบ bounded ที่ส่งเข้า Pulse v2; projection นี้ยังเป็น
advisory input และไม่ทำให้ Pulse หรือ UI เป็น authority ส่วน
`.tmux-teams/pulse.json` ยังคงเป็น Pulse snapshot SSOT เพียงไฟล์เดียว ห้ามสร้าง
`pulse-v2.json` หรือ persisted compat-v1 snapshot เพิ่ม
