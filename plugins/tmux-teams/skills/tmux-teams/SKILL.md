...
| `scripts/acp-dispatch.mjs` | the operator's way in to that leg — detaches it into its own process group so the calling shell's cap is not the lane's deadline, and `status <cwd> <task-id>` reads back liveness, outbox and the resume command |
