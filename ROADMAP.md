...
Current release: **0.43.0** — the version stamped in this tree, in flight on
a pull request and not yet tagged. `main` carries **v0.42.0**, tagged at the
MERGED sha `1ab3109` (not the branch tip `300f293` — those are different
...
Four items set by Master 2026-08-22 — two inventory, one conformance, one new
capability that has to be written rather than vendored — plus a fifth added by
direct instruction 2026-08-23 (see below).
...
| 5 | `test-quality` skill: contract-only MVP | **shipped this branch** — awaiting release |

**Item 5 provenance (2026-08-23):** `test-quality`, written here. Contract-only
MVP: evidence semantics for CRAP / mutation adequacy / coverage, policy decision
separate from evidence state, no runner, no schema file, no baseline store, no
dependency. Architecture reviewed by AGY (`gemini-3.7-flash-high`) and Codex
(`gpt-5.6-sol[max]`) read-only lanes before implementation; both answered
PROCEED WITH CHANGES and the convergent blockers (state-plane separation,
method-level granularity, verify-command-as-audit-trail, denominator
reconciliation) are in the shipped contract.
