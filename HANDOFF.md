# HANDOFF — tmux-teams · **v0.30.0 ปล่อยแล้ว · MQ/pub-sub เดินจริง 5 ใน 6 ช่อง** (2026-08-15)
...
1. **Claude ACP อ้างเลย์เอาต์โปรไฟล์ที่ต่างกันทุกเครื่อง** (Master ยกเมื่อ 15 ส.ค.)
   · เครื่องนี้: wrapper ตั้ง `CLAUDE_CONFIG_DIR` แล้ว `exec claude`
   · เครื่อง Linux: `~/.claude/profiles/<ชื่อ>.json` + `.env` แล้ว `exec claude --settings`
   · `review-profiles.mjs` ประกาศเลย์เอาต์เดียว — **ของเครื่องนี้**
   · เหตุผลที่มันอ่านไฟล์: ตรึง endpoint ในโพรเซสแม่ → `familyProvenKey` `::pinned:<host>`
   · **ทางที่วัดแล้วว่าเป็นไปได้:** adapter ประกาศเวนเดอร์จริงต่อ alias เอง
     (`opus → qwen3.8-max[1m]` · `sonnet → deepseek-v4-flash-0731[1m]`)
     → ตรวจ **"อะไรตอบ"** แทน **"config ชี้ไปไหน"** · พกพาได้และแรงกว่าเดิม
   · ⚠️ เป็นการเปลี่ยนกฎที่ตัดสินว่ารีลีสผ่านหรือไม่ — **อย่าทำเงียบ ๆ**
2. **เบรกแปดตัวใน `loop-runner.mjs` ยังอยู่ครบ** — ที่นั่ง PM ที่มันยืนแทนมีแล้ว
   **แต่ถอดได้ต่อเมื่อมีหลักฐานรายตัว**ว่า WIP hold ครอบสิ่งที่มันเคยครอบ
...
4. **`claude-qwen` ฝังคีย์เป็นค่า default ในตัวสคริปต์** — ควรย้ายไปไฟล์ env
