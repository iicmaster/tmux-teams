...
- **fan out เอเจนต์ที่แต่ละตัวรันชุดเทสต์เอง** — 5 ตัว × (apply + suite + revert +
  suite) = 15 รอบพร้อมกันบน 8 คอร์ · **load 28.17 · 42 นาทีไม่มีใครเสร็จสักตัว** ·
  ที่แพงกว่าเวลาคือ **ตัวเลขที่วัดได้ต้องทิ้งทั้งรอบ** เพราะ phase ที่มีไว้วัด
  กลับไปวัดการแย่งซีพียูแทน · กติกาอยู่ใน `CLAUDE.md` แล้ว: **ขนานความคิด
  อนุกรมการวัด** · และตรวจว่าเอเจนต์ค้างไหมจาก mtime ของ transcript + `ps` +
  `/proc/loadavg` **ไม่ใช่จากเวลาที่ผ่านไป**
