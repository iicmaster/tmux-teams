...
## หลัง v0.15.0 — งานหน้ากราฟ 14 commit (2026-08-06)

**v0.15.0 ปล่อยครบทุกช่องทางแล้ว** (tag, GitHub release, marketplace, plugin
cache) · หลังจากนั้นเป็นงานหน้ากราฟล้วน สั่งทีละข้อโดย Master · `bdd3634`

| เรื่อง | ผลที่วัดได้ |
|---|---|
| halo กระพริบเร็วขึ้น 2 เท่า | 2.6s → 1.3s · stagger ตามเป็นสัดส่วน |
| ปุ่ม `fit` | 28px → 39.6px เท่าพี่น้องทั้งเจ็ด · ทุกปุ่มมี tooltip |
| ภาษาเส้น (กฎ Master) | ประ+dashflow = ในทีม+escalate · ทึบ+comet = ข้ามทีม |
| `blocked` เป็นสถานะของตัวเอง | เดิมรายงานเป็น `delivered` ขัดกับ legend |
| contrast บนการ์ด control/team | 1.67:1 → ผ่าน AA ด้วย on-ink trio ต่อธีม |
| เส้นจบกลับหา PM | `pull` + comet ตามคำตัดสิน Master |
| การ์ดที่นั่ง | เหลือรูปแบบเดียว ไม่ขึ้นกับซูม |
| เส้นในทีมเกาะขอบ | 48 ปลายเส้น ไม่มีจุดไหนอยู่กึ่งกลาง |
| น้ำหนักเส้นตามการเคลื่อนไหว | ทั้งประและทึบ: จางตอนนิ่ง เข้มตอนมีอะไรวิ่ง |
| ระยะแนวตั้งในทีม | 122/136/142 → 183/204/213 |
| bow ของ assign/judge | 30/18 → 0 (port แยกเส้นให้แล้ว) |

### รูปแบบความผิดพลาดที่เกิดสามครั้งในเรื่องเดียวกัน — อ่านก่อนแตะเรขาคณิต

**ค่าคงที่ที่เคยแก้ปัญหาจริง รอดมาถึงโลกที่เหตุผลของมันหายไปแล้ว แล้วกลายเป็น
ปัญหาใหม่** · เกิดสามครั้ง: `CARD_W/2` ที่จ่ายซ้ำ · bow ที่ไม่หดตามระยะเมื่อ
port ทำให้เส้นสั้นลง · bow ที่หมดหน้าที่หลังมี port · **สมมติว่ามีตัวที่สี่**

### สิ่งที่ Master จับได้สี่ครั้งติด และบทเรียนที่เขียนลง CLAUDE.md แล้ว

รายงานว่าเสร็จสี่ครั้ง ผิดทั้งสี่ครั้ง suite เขียวทั้งสี่ครั้ง · **การวัดพิสูจน์
ปริมาณกับตรรกะ พิสูจน์รูปทรงไม่ได้ · ภาพพิสูจน์รูปทรง พิสูจน์การเคลื่อนไหวไม่ได้
· ต้องทำทั้งสองอย่าง** และรายงาน**ผลลัพธ์ที่วัดได้** ไม่ใช่ค่าคงที่ที่แก้ไป

### ค้างไว้

- **สามโหนดของ control team เรียงแนวนอนด้านบน** — Master สั่งแล้ว ยังไม่ทำ ·
  ชนเทส `the control team is one node — its three seats are jobs, not stations`
  · โหนดสร้างที่ `graph-tour.mjs:105` · `agentsOf()` คืน `[team:control]`
  อย่างเดียวโดยตั้งใจ · `how-it-works.md` บันทึกว่า `pm_audit` ไม่เคยถูก
  dispatch บนเส้นทางปกติ ซึ่งเกี่ยวกับ "จะเห็นว่าโหนดไหนทำงาน"
- **finding UX อีก 12 ข้อ** จากรีวิวสามมุม (ประโยคบอกเหตุผลถูกตัดครึ่ง ·
  คำศัพท์ที่ไม่เคยนิยาม · โครงสร้าง a11y)
- **submodule pin** ใน `~/agent-skills` commit แล้วที่ `6b29718` **ยังไม่ push**
  และตามหลังหลายคอมมิต · คนละโปรเจกต์ ต้องขอ Master
- **รีวิวปิดงานโดย AGY** — ยิงไปแล้ว ดูผลที่ `<scratch>/agy/.mailbox-out/v015-post-release`
