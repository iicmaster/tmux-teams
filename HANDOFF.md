# HANDOFF — tmux-teams · **v0.31.0 อยู่ใน PR รอบอทรีวิว · ยังไม่ merge** (2026-08-16)

> เขียนให้ **agent** อ่านแล้วลงมือต่อได้ทันที
> **ห้ามเขียนตัวเลขที่เปลี่ยนได้ลงในไฟล์นี้ — เขียนคำสั่งแทน**

## 🔴 อ่านบรรทัดนี้ก่อน — รีลีสค้างอยู่ที่เดียว

```
PR ปล่อย v0.31.0   branch release/0.31.0 → main   (ยังไม่ merge)
CI                 เขียวบน head ปัจจุบัน
แผงสามโมเดล        3/3 · finding 0 · รันสามรอบ (บันทึกอยู่ใน PR body)
codex bot          ⛔ "You have reached your Codex usage limits for code reviews"
```

**กติกาใหม่ของ Master (16 ส.ค.): รีลีสไปเป็น PR เท่านั้น · merge ได้เมื่อ CI เขียว *และ*
บอท codex รีวิว** · เขียนลง `CLAUDE.md` ขั้น 7 แล้ว
· **คอมเมนต์ของบอทไม่ใช่รีวิว** — ตัวที่มีอยู่คือคำบอกว่าโควตาหมด = ผู้ตรวจไม่มา
ไม่ใช่ผ่านและไม่ใช่ไม่ผ่าน · เช็ค review **state** ไม่ใช่เช็คว่ามีคอมเมนต์
· โควตากลับมาแล้วบอทไม่รีวิวเอง — ต้อง re-request หรือ push คอมมิตใหม่
· **หลัง merge: tag ที่ sha ของ `main` หลัง merge ไม่ใช่ปลาย branch**

```bash
gh pr view 66 --json reviews -q '.reviews[]|.author.login+" "+.state'   # ว่าง = บอทยังไม่รีวิว
gh pr checks 66
```

**สิ่งที่รออยู่หลัง merge:** tag + GitHub release (แนบตาราง `Gate: 3/3` ทุกเลน)
· `claude plugin marketplace update` + `update` · **ปักหมุด submodule ใน `~/agent-skills`
ต้องขอ Master ก่อน** (คนละเรพ คนละบัญชี `ngs-th`)
