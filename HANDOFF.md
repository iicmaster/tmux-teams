# HANDOFF — tmux-teams · **v0.19.0 ปล่อยแล้ว · แผงเดิน 10 รอบ** (2026-08-11)
...
- **อิชชูเปิดค้างใบเดียวโดยตั้งใจ** — เลน claude-routed รันใน bwrap ไม่ได้เพราะ
  ...
  อยู่รอบครึ่ง เพราะสามชั้นสุดท้ายอยู่ใน `0228f6b` ที่ไม่มีใครกลับมาพับเข้าเอกสาร)
  · **transport ทำงานครบวงจรแล้ว** — 9 ส.ค. เลน zai เดิน
  `config → closed(127) → closed(1) → closed(1) → protocol → review` รัน 4 นาที 8 วินาที
  **glm-5.2 อ่าน diff และตอบจริง** · ที่ยังเปิดอยู่ **ไม่ใช่ sandbox แล้ว** — เป็นสามอย่าง:
  เลนที่ตอบร้อยแก้วแทน JSON เอกสารเดียว · `qwen` ที่ไม่เคยถูก provision บนเครื่องทดสอบ ·
  และการรันสามเลนพร้อมกันที่ยังไม่มีใครลอง
...
5. push (ขออนุญาตก่อน) → เช็คไม่มี ahead → `gh run watch <id> --exit-status` **ห้าม tag ตัวแดง**
6. `git tag` + `gh release create --notes-file` (**ห้าม `printf`** และ**ห้าม backtick ใน `-m`** —
   เชลล์รันมันเป็นคำสั่ง ข้อความหายไปสองวลีมาแล้ววันนี้ · ข้อความยาวทุกชนิดไปทางไฟล์)
7. `claude plugin marketplace update` + `claude plugin update`
8. **ปักหมุด submodule ใน `~/agent-skills` แล้ว push** (บัญชี `ngs-th` — เรพนั้นเป็นของ ngs-th
