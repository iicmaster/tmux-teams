...
### ⚠️ ยังเผยแพร่ไม่ได้ — artifact-sftp MCP เสียที่ชั้น MCP

```
artifact_sftp_setup_status  →  exit_code: 10   ← รหัสของการ์ดใน setup.sh เป๊ะ
ARTIFACT_SFTP_MCP_CALL=1 bash setup.sh --status  →  exit 0 · READY   ← เครื่องพร้อม
โพรเซสฝั่ง Claude  PID 13375 (12 ส.ค. 22:50) · 33978 (13 ส.ค. 01:12)  ← เก่ากว่าคอมมิตล่าสุด
โพรเซสฝั่ง Codex   ~15 ตัวสะสม
`/mcp` reconnect ไม่ได้สปอว์นตัวใหม่
```

**การตั้งค่าเครื่องพร้อม · ซอร์สมีสายไฟครบ (`service.py:88`) · ที่ตอบเราคือโพรเซสเก่า**
· เรพนั้นมี branch `fix/mcp-only-agent-routing` ค้างอยู่ — **อย่าเข้าไปแก้ทับงานคนอื่น**
· วิธีทดสอบสมมติฐาน: `kill 13375 33978` แล้วเรียก tool ใหม่ · ถ้าใช้ได้ → เผยแพร่แล้ว
`node scripts/roadmap-gate.mjs --record <url>`
...
**สิ่งเดียวที่ค้างจริงและมีเกตจับอยู่: `roadmap-gate.mjs` ตอบ `2`** — หน้ายังไม่ถูกเผยแพร่
เพราะ artifact-sftp MCP เสีย (ดูหัวข้อ roadmap ข้างบน) · **รีลีสหน้าจะสะดุดกับมันเอง**
